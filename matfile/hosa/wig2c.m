function [wx,waxis] = wig2c (x0,nfft,sigma,flag)
%WIG2C	 Computes the Wigner Distribution with Choi-Williams filtering
...
[m, n] = size(x0);
if (min(m,n) ~= 1)
   disp(['wig2c: input argument x is a ',int2str(m),' by ',int2str(n), ...
         ' array'])
...
if (nfft < 2*lx)
   disp(['WIG2C: FFT length must exceed twice the signal length'])
...
xlabel('frequency')
title(['CWS s=',num2str(sigma)])
set(gcf,'Name','Hosa WIG2C')
